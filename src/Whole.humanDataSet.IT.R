...
data.ob.umap <- data.ob.umap %>% filter(pj %in% c("EBD2","IBD2","nBGuo")) %>% filter(cellType!="EM")  %>% bind_rows(data.ob.umap %>% filter(pj %in% c("nicolBla")) %>% filter(!EML %in% c("okae_bts5","naive_H9","primed_H9")))%>% mutate(cluster_EML=ifelse(seurat_clusters %in% c("C1","C7","C9"),"ELC","Undef")) %>% mutate(cluster_EML=ifelse(seurat_clusters %in% c("C4"),"HLC",cluster_EML))%>% mutate(cluster_EML=ifelse(seurat_clusters %in% c("C3","C12"),"TLC",cluster_EML))%>% mutate(cluster_EML=ifelse(seurat_clusters %in% c("C5","C6"),"MeLC",cluster_EML)) %>% mutate(cluster_EML=ifelse(seurat_clusters %in% c("C0","C8","C10"),"AMLC",cluster_EML)) %>% bind_rows(data.ob.umap %>% filter(!pj %in% c("EBD2","IBD2","nicolBla")) %>% filter(!(pj=="nBGuo" & cellType!="EM")) %>% mutate(cluster_EML=rename_EML)) %>% bind_rows(data.ob.umap %>% filter(pj %in% "nicolBla") %>% filter(EML %in% c("okae_bts5","naive_H9","primed_H9")) %>% mutate(cluster_EML=rename_EML))
