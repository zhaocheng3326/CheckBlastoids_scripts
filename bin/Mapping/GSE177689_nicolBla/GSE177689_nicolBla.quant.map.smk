...
rule fq_dump:
  input:
    sra=config['RD']+"{sample}.sra",
  output:
    fq1=temp(config['RD']+"{sample}_1.fastq.gz"),
    fq2=temp(config['RD']+"{sample}_2.fastq.gz"),
  shell:
    "fastq-dump --split-3  {input.sra} --gzip  -O $(dirname {output.fq1})"

rule STAR_mapping:
  input:
    fq1=config['RD']+"{sample}_1.fastq.gz",
    fq2=config['RD']+"{sample}_2.fastq.gz",
    GTF=config['GTF'],
